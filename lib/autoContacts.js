...
function normalize(value) {
  ...
  for (const jid of Array.isArray(state.sent) ? state.sent : []) {
    ...
    if (clean && !contacts[clean]) {
      contacts[clean] = { name: 'New Contact • 𝐌𝐒𝐄𝐋𝐀-𝐂𝐇𝐔𝐈-𝐗𝐌𝐃', addedAt: Date.now() };
    }
  }
  const bounded = Object.fromEntries(Object.entries(contacts)
    ...
    .slice(-5000)
    .map(([jid, contact]) => [jid, {
      name: String(contact.name || 'New Contact • 𝐌𝐒𝐄𝐋𝐀-𝐂𝐇𝐔𝐈-𝐗𝐌𝐃').slice(0, 80),
...
async function registerAutoContact(jid, name = 'New Contact • 𝐌𝐒𝐄𝐋𝐀-𝐂𝐇𝐔𝐈-𝐗𝐌𝐃') {
  if (!jid) return null;
  const state = await getAutoContactState();
